...
// ============================================================================
// Upload Document (Create metadata, get signed URL for upload)
// ============================================================================
...
export async function uploadDocumentAction(data: unknown) {
  try {
    ...
    if (!profile.permissions?.includes('documents.upload')) {
      return { error: 'Permission denied: documents.upload', code: 'FORBIDDEN' };
    }

    // File type validation (allow only safe types)
    ...
    // Generate storage path (tenant-isolated)
...
export async function deleteDocumentAction(data: unknown) {
  try {
    ...
    if (!profile.permissions?.includes('documents.delete')) {
      return { error: 'Permission denied: documents.delete', code: 'FORBIDDEN' };
    }
...
export async function approveDocumentAction(data: unknown) {
  try {
    ...
    if (!profile.permissions?.includes('documents.update')) {
      return { error: 'Permission denied: documents.update', code: 'FORBIDDEN' };
    }
...
export async function getDocumentsAction(employeeId?: string) {
  try {
    ...
    if (employeeId) {
      // Get documents for specific employee (HR only)
